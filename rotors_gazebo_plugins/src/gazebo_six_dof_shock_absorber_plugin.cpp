...
void GazeboShockAbsorber::OnUpdate(const common::UpdateInfo& _info)
{
  ...
  math::Vector3 delta_velocity = parent_link_->GetWorldLinearVel()
      - child_link_->GetWorldLinearVel();
  ...
  math::Vector3 torque_w = rotational_damper_constant_*delta_angular_velocity_world_frame;

  child_link_->AddRelativeTorque(initial_child_pos.rot.RotateVectorReverse(torque));
  child_link_->AddTorque(torque_w);
  parent_link_->AddRelativeTorque(-torque);
  parent_link_->AddTorque(-torque_w);
}
