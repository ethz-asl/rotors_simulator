...
void GazeboForceSensorPlugin::Load(physics::ModelPtr _model,
                                   sdf::ElementPtr _sdf) {
  ...
  if (link_ == NULL)
    gzthrow("[gazebo_force_sensor_plugin] Couldn't find specified link \""
            << link_name_ << "\".");

  joint_ = model_->GetJoint(link_name_);
  if (joint_ == NULL)
    gzthrow("[gazebo_force_sensor_plugin] Couldn't find specified joint \""
            << link_name_ << "\".");
  ...
  getSdfParam<std::string>(_sdf, "forceSensorTopic", force_sensor_pub_topic_,
                           force_sensor_pub_topic_);
  getSdfParam<std::string>(_sdf, "forceSensorTruthTopic",
                           force_sensor_truth_pub_topic_,
                           force_sensor_truth_pub_topic_);
  getSdfParam<std::string>(_sdf, "wrenchVectorTopic", wrench_vector_pub_topic_,
                           wrench_vector_pub_topic_);
  getSdfParam<std::string>(_sdf, "publishFrameId", publish_frame_id_,
                           publish_frame_id_);
  getSdfParam<std::string>(_sdf, "parentFrameId", parent_frame_id_,
                           parent_frame_id_);
  getSdfParam<std::string>(_sdf, "referenceFrameId", reference_frame_id_,
                           reference_frame_id_);
  getSdfParam<ignition::math::Vector3d>(_sdf, "noiseNormalLinearForce",
                                        noise_normal_linear_force, zeros3);
  getSdfParam<ignition::math::Vector3d>(_sdf, "noiseNormalTorque",
                                        noise_normal_torque, zeros3);
  getSdfParam<ignition::math::Vector3d>(_sdf, "noiseUniformLinearForce",
                                        noise_uniform_linear_force, zeros3);
  getSdfParam<ignition::math::Vector3d>(_sdf, "noiseUniformTorque",
                                        noise_uniform_torque, zeros3);
  ...
  getSdfParam<double>(_sdf, "cutoffFrequency", cutoff_frequency_,
                      cutoff_frequency_);
  getSdfParam<bool>(_sdf, "linForceMeasEnabled", lin_force_meas_enabled_,
                    lin_force_meas_enabled_);
  getSdfParam<bool>(_sdf, "torqueMeasEnabled", torque_meas_enabled_,
                    torque_meas_enabled_);
  getSdfParam<bool>(_sdf, "dispWrenchVector", disp_wrench_vector_,
                    disp_wrench_vector_);

  parent_link_ = model_->GetLink(parent_frame_id_);
  if (parent_link_ == NULL && parent_frame_id_ != kDefaultParentFrameId) {
    gzthrow(
        "[gazebo_force_sensor_plugin] Couldn't find specified parent link \""
        << parent_frame_id_ << "\".");
  }

  reference_link_ = model_->GetLink(reference_frame_id_);
  if (reference_link_ == NULL &&
      reference_frame_id_ != kDefaultReferenceFrameId) {
    gzthrow(
        "[gazebo_force_sensor_plugin] Couldn't find specified reference frame "
        "\""
        << reference_frame_id_ << "\".");
  }
  ...
  if (lin_force_meas_enabled_) {
    ...
    linear_force_u_[0] = UniformDistribution(-noise_uniform_linear_force[0],
                                             noise_uniform_linear_force[0]);
    linear_force_u_[1] = UniformDistribution(-noise_uniform_linear_force[1],
                                             noise_uniform_linear_force[1]);
    linear_force_u_[2] = UniformDistribution(-noise_uniform_linear_force[2],
                                             noise_uniform_linear_force[2]);

    // Linear forces publisher
    lin_force_sensor_pub_ =
        node_handle_->advertise<geometry_msgs::Vector3Stamped>(
            force_sensor_pub_topic_ + "/linear", 10);
    lin_force_sensor_truth_pub_ =
        node_handle_->advertise<geometry_msgs::Vector3Stamped>(
            force_sensor_truth_pub_topic_ + "/linear", 10);
  }
  ...
  if (torque_meas_enabled_) {
    ...
    torque_u_[0] =
        UniformDistribution(-noise_uniform_torque[0], noise_uniform_torque[0]);
    torque_u_[1] =
        UniformDistribution(-noise_uniform_torque[1], noise_uniform_torque[1]);
    torque_u_[2] =
        UniformDistribution(-noise_uniform_torque[2], noise_uniform_torque[2]);

    // Torques publisher
    ang_force_sensor_pub_ =
        node_handle_->advertise<geometry_msgs::Vector3Stamped>(
            force_sensor_pub_topic_ + "/angular", 10);
    ang_force_sensor_truth_pub_ =
        node_handle_->advertise<geometry_msgs::Vector3Stamped>(
            force_sensor_truth_pub_topic_ + "/angular", 10);
  }

  if (disp_wrench_vector_) {
    // Wrench vector publisher for RViz visualization
    wrench_vector_pub_ = node_handle_->advertise<geometry_msgs::WrenchStamped>(
        wrench_vector_pub_topic_, 10);
  }

  // Listen to the update event. This event is broadcast every simulation
  // iteration.
  updateConnection_ = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&GazeboForceSensorPlugin::OnUpdate, this, _1));
}
...
void GazeboForceSensorPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  if (reference_frame_id_ != kDefaultReferenceFrameId &&
      reference_frame_id_ != parent_frame_id_) {
  ...
  /* FORCE PARSING */
  // The wrench vectors consist of two vectors representing the internal
  // resultant force and torque applied on force sensor joint by child link
  // (i.e. force sensor itself). The values of the two 3D vectors are the
  // coordinates in parent frame coordinates system of the tip of an arrow
  // starting at the origin of parent frame.
  ...
  if (prev_sim_time_ == 0.0) {
    ...
    sensor_data_filter_.reset(new FirstOrderFilterJointWrench(
        time_constant_, time_constant_, joint_wrench_));
  } else {
    // Smooth row data by first order filter.
    sampling_time_ = _info.simTime.Double() - prev_sim_time_;
    joint_wrench_ =
        sensor_data_filter_->updateFilter(joint_wrench_, sampling_time_);
  }
  prev_sim_time_ = _info.simTime.Double();

  // Get forces applied to force sensor joint from child link (i.e. force
  // sensor) in parent frame coordinates.
  force = -joint_wrench_.body1Force;

  // Get torques applied to sensor joint from child link (i.e. force sensor) in
  ...
  if (gazebo_sequence_ % measurement_divisor_ == 0) {
    ...
    wrench_msg_.header.stamp.sec =
        (world_->SimTime()).sec + ros::Duration(unknown_delay_).sec;
    wrench_msg_.header.stamp.nsec =
        (world_->SimTime()).nsec + ros::Duration(unknown_delay_).nsec;
    ...
    if (publish_forces)
      wrench_queue_.push_back(
          std::make_pair(gazebo_sequence_ + measurement_delay_, wrench_msg_));
  }
  ...
  if (gazebo_sequence_ == wrench_queue_.front().first) {
    ...
    if (lin_force_meas_enabled_) {
      ...
      geometry_msgs::Vector3StampedPtr true_lin_forces(
          new geometry_msgs::Vector3Stamped);
      geometry_msgs::Vector3StampedPtr noisy_lin_forces(
          new geometry_msgs::Vector3Stamped);
      ...
      linear_force_n << linear_force_n_[0](random_generator_) +
                            linear_force_u_[0](random_generator_),
          linear_force_n_[1](random_generator_) +
              linear_force_u_[1](random_generator_),
          linear_force_n_[2](random_generator_) +
              linear_force_u_[2](random_generator_);
      // Fill linear force fields.
      noisy_lin_forces->vector.x =
          true_lin_forces->vector.x + linear_force_n[0];
      noisy_lin_forces->vector.y =
          true_lin_forces->vector.y + linear_force_n[1];
      noisy_lin_forces->vector.z =
          true_lin_forces->vector.z + linear_force_n[2];
    ...
    if (torque_meas_enabled_) {
      ...
      geometry_msgs::Vector3StampedPtr true_torques(
          new geometry_msgs::Vector3Stamped);
      geometry_msgs::Vector3StampedPtr noisy_torques(
          new geometry_msgs::Vector3Stamped);
      ...
      torque_n << torque_n_[0](random_generator_) +
    ...
    tf::Quaternion tf_q_P_C(gazebo_pose.Rot().X(), gazebo_pose.Rot().Y(),
                            gazebo_pose.Rot().Z(), gazebo_pose.Rot().W());
    tf::Vector3 tf_p_P_C(gazebo_pose.Pos().X(), gazebo_pose.Pos().Y(),
                         gazebo_pose.Pos().Z());
    tf_ = tf::Transform(tf_q_P_C, tf_p_P_C);
    transform_broadcaster_.sendTransform(
        tf::StampedTransform(tf_, wrench_queue_.front().second.header.stamp,
                             parent_frame_id_, link_name_));
    if (parent_frame_id_ != kDefaultParentFrameId &&
        reference_frame_id_ != parent_frame_id_) {
      // Transformation between parent link and reference frame.
      tf::Quaternion tf_q_R_P(
          gazebo_parent_pose.Rot().X(), gazebo_parent_pose.Rot().Y(),
          gazebo_parent_pose.Rot().Z(), gazebo_parent_pose.Rot().W());
      tf::Vector3 tf_p_R_P(gazebo_parent_pose.Pos().X(),
                           gazebo_parent_pose.Pos().Y(),
                           gazebo_parent_pose.Pos().Z());
    ...
    if (reference_frame_id_ != kDefaultReferenceFrameId) {
      ...
      tf::Quaternion tf_q_W_R(
          gazebo_reference_pose.Rot().X(), gazebo_reference_pose.Rot().Y(),
          gazebo_reference_pose.Rot().Z(), gazebo_reference_pose.Rot().W());
      tf::Vector3 tf_p_W_R(gazebo_reference_pose.Pos().X(),
                           gazebo_reference_pose.Pos().Y(),
                           gazebo_reference_pose.Pos().Z());
      tf_.setOrigin(tf_p_W_R);
      tf_.setRotation(tf_q_W_R);
      transform_broadcaster_.sendTransform(
          tf::StampedTransform(tf_, wrench_queue_.front().second.header.stamp,
                               "world", reference_frame_id_));
    }
...
void GazeboForceSensorPlugin::PrintJointWrench(physics::JointWrench jw_) {
  ROS_INFO(
      "[gazebo_force_sensor_plugin]\nJoint Wrench Forces:\n  Fx = %f  Fy = %f  "
      "Fz = %f\nJoint Wrench Torques:\n  Tx = %f  Ty = %f  Tz = %f\n",
      jw_.body1Force[0], jw_.body1Force[1], jw_.body1Force[2],
      jw_.body1Torque[0], jw_.body1Torque[1], jw_.body1Torque[2]);
}
